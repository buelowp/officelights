...
DiscoStrobe::DiscoStrobe(CRGB *s, CRGBPalette16 pal)
{
...
//
void DiscoStrobe::discoWorker(uint8_t dashperiod, uint8_t dashwidth, int8_t dashmotionspeed, uint8_t stroberepeats, uint8_t huedelta)
{
    ...
    drawRainbowDashes( sStartPosition, m_numLeds-1, 
  ...
  uint8_t huestart, uint8_t huedelta, uint8_t saturation, uint8_t value)
{
    ...
    for( uint16_t i = startpos; i <= lastpos; i += period) {
        ...
        CRGB color = ColorFromPalette(m_CurrentPalette, hue, value, NOBLEND);
...
void DiscoStrobe::delayToSyncFrameRate( uint8_t framesPerSecond)
{
    ...
    if (msdelta < mstargetdelta) {
        QThread::msleep(mstargetdelta - msdelta);
    }
    msprev = mscur;
}

void DiscoStrobe::discoStrobeWithPalette()
{
...
void DiscoStrobe::action()
{
    ...
    m_strobePhase++;
    if (m_strobePhase >= kStrobeCycleLength ) { 
        m_strobePhase = 0; 
    }
